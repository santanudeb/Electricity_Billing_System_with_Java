...
public class CreateID extends JFrame implements ActionListener
{
	...
	public CreateID()
	{
		...
		panel=new JLabel(new ImageIcon("Image/create.jpg"));
	...
	public void actionPerformed(ActionEvent e) 
	{
		...
		if(e.getSource()==txt||e.getSource()==submit)
		{
			if(txt.getText()!=null)
			{
				int n=JOptionPane.showConfirmDialog(null, "Are you sure to Create Consumer ID : "+id+"?", "Confirm", JOptionPane.YES_NO_OPTION);
				if(n==0)
				{
					Cruding c=new Cruding();
					try 
					{
						boolean b=c.createid(id);
						if(b)
						{
							JOptionPane.showMessageDialog(null, "Created", "Done", JOptionPane.INFORMATION_MESSAGE);
